...
class ActorCritic(nn.Module):
    ...
    def forward(self, state):
        ...
        state = F.relu(self.affine2(state))
...
def train():
    ...
    for i in range(140):
        for j in range(140):
            if i < 20 or  80> i > 60 or i > 120 or j >120 or j < 20:
                state[0] = -7+ j/10
                state[1] = -i/10+7
                z[i,j] = critic(state)[0]

    fig, ax = plt.subplots()
    im = ax.pcolormesh(X, Y, z, cmap='inferno',vmin=0,vmax=1.6)
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
